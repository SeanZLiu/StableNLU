...
def main():
    ...
    if args.get_bert_output:
        if args.task == 'mnli':
            out = load_mnli(is_train=True)[:20000]
            eval_datasets = [("mnli_bert_20000", out)]
        elif args.task == 'fever':
            if args.do_eval_on_train:
    ...
    for ix, (name, eval_examples) in enumerate(eval_datasets):
        ...
        # todo final
        shallow_features_1 = pickle.load(open(args.shallow_feature_file, 'rb'))
        shallow_features_2 = pickle.load(open("shallow_features/shallow_qqp_train1.pkl",'rb'))
        shallow_features = {key: shallow_features_1[key] + shallow_features_2[key] for key in shallow_features_1}
