...
class ReweightBaseline(ClfDistillLossFunction):
    def __init__(self, weights):
        super(ReweightBaseline, self).__init__()
    ...
    def forward(self, logits, labels):
        ...
        # import pdb
        # pdb.set_trace()
        return (self.weights.squeeze() * loss).sum() / self.weights.sum()


class ReweightPod(ClfDistillLossFunction):
    def __init__(self, weights, shallow_probs):
        super(ReweightPod, self).__init__()
        self.weights = weights
        self.shallow_probs = shallow_probs + 0.1

    def forward(self, logits,labels):
        logits = F.log_softmax(logits.float()) # In case we were in fp16 mode
        shallow_probs = self.shallow_probs.log()[:,0, :] * self.weights
        logits = logits + shallow_probs
        loss = F.cross_entropy(logits, labels, reduction='none')
        # import pdb
        # pdb.set_trace()
        return loss.sum()
...
class Pod(ClfDistillLossFunction):
    def __init__(self, shallow_probs, avg=False):
        super(Pod, self).__init__()   
        self.shallow_probs = shallow_probs + 0.001
        self.avg = avg

    def forward(self, logits,labels):
        logits = F.log_softmax(logits.float()) # In case we were in fp16 mode
        if not self.avg:
            shallow_probs = self.shallow_probs.log()[:,0, :]
        else:
            shallow_probs = (torch.prod(self.shallow_probs, 1) ** (1 / self.shallow_probs.shape[1])).log()
        logits = logits + shallow_probs
        loss = F.cross_entropy(logits, labels, reduction='none')
        # import pdb
        # pdb.set_trace()
...
class ExpPod(ClfDistillLossFunction):
    def __init__(self, shallow_probs, avg=False):
        super(ExpPod, self).__init__()   
        self.shallow_probs = shallow_probs + 0.001
        self.avg = avg

    def forward(self, logits,labels):
        # logits = F.softmax(logits.float()) # In case we were in fp16 mode
        logits = logits.float() # In case we were in fp16 mode
        if not self.avg:
            # shallow_probs = self.shallow_probs[:,0, :]
            shallow_probs = self.shallow_probs[:,0, :].log()
        else:
            shallow_probs = (torch.prod(self.shallow_probs, 1) ** (1 / self.shallow_probs.shape[1]))
        logits = logits + shallow_probs
        loss = F.cross_entropy(logits, labels, reduction='none')
        # import pdb
        # pdb.set_trace()
...
class ReweightLoss(ClfDistillLossFunction):
