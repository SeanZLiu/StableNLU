...
from onmt.clf_distill_loss_functions import ClfDistillLossFunction


class BertDistill(BertPreTrainedModel):
    """Pre-trained BERT model that uses our loss functions"""

    def __init__(self, config, num_labels, method_combine='add'):
        ...
        self.method_combine = method_combine
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.aux_linear = nn.Linear(config.hidden_size, config.hidden_size)
        self.config = config
        # self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, disturb=False):
        _, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False) # , disturb=disturb
