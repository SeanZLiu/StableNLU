"""
Script to train BERT on MNLI and evaluate MNLI and dev using the feature file directly,instend of loading the original dataset file.
从更改后的bert文件（bert_distill_feature）中引入了forward
"""
import pickle
import argparse
import json
...
# todo
from bert_distill_feature import BertDistill,LinearNet
...
def collate_input_features(batch: List[InputFeatures]):
    ...
    return example_ids, input_ids, mask, segment_ids, label_ids, bias, teacher_probs,shallow_feature
...
def main():
    ...
    parser.add_argument("--shallow_feature_file", type=str, default=None, help="the pkl file to save the shallow feature.")
    parser.add_argument("--final_feature",type=str, default=None, help="the pkl file to save final features ,which have already fused "
    ...
    if args.task == 'qqp':
        ...
        label_num = 3
    if args.linear:
        model = LinearNet(input_size=768,num_labels=3)
        logging.info("Use Linear model.")
    else:
    ...
    if args.do_train:
        ...
        # todo
        shallow_features = handle_shallow_feature(shallow_file_path=args.shallow_feature_file, k=args.shallow_model_num)
        train_features: List[InputFeatures] = convert_examples_to_features(train_examples, args.max_seq_length, tokenizer, shallow_features=shallow_features)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch", ncols=100):
            ...
            for step, batch in enumerate(pbar):
                ...
                if bias_map is not None:
                    example_ids, input_ids, input_mask, segment_ids, label_ids, bias, teacher_probs,bias_features = batch
                else:
                    bias = None
                    example_ids, input_ids, input_mask, segment_ids, label_ids,bias_features = batch

                if args.linear:
                    logits, loss = model(labels=label_ids, bias_features=bias_features)
                else:
                    logits, loss = model(input_ids, segment_ids, input_mask, label_ids, bias,
                                         teacher_probs, bias_features)
                total_steps += 1
    ...
    for ix, (name, eval_examples) in enumerate(eval_datasets):
        ...
        shallow_path=""
        if name =="mnli_dev_m":
            shallow_path = "shallow_features/shallow_mnli_dev_ernie.pkl"
        else:
            shallow_path = "shallow_features/shallow_mnli_hans_ernie.pkl"
        shallow_features = handle_shallow_feature(shallow_file_path=shallow_path,k=args.shallow_model_num)
        ...
        for example_ids, input_ids, input_mask, segment_ids, label_ids,bias_features in tqdm(eval_dataloader,
                                                                               desc="Evaluating",
                                                                               ncols=100):
            ...
            bias_features=bias_features.to(device)

            with torch.no_grad():
                if args.linear:
                    logits = model(bias_features=bias_features)
                else:
        ...
        if "hans" in name:
            ...
            max_id = np.argmax(probs, axis=1)
            for ith, i in enumerate(max_id):
                probs[ith, i] = 0
            probs[:, 0] = probs[:, [0, 2]].max(axis=1)
            probs = probs[:, :2]
