...
def main():
    ...
    if args.get_bert_output:
        # todo 重置需要评估的datasets 只留一个20000样本的mnli子集
        out = load_mnli(is_train=True,sample=20000)[:20000]
        eval_datasets = [("mnli_bert", out)]
    ...
    for ix, (name, eval_examples) in enumerate(eval_datasets):
        ...
        for example_ids, input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader,
            ...
            with torch.no_grad():
                ...
                if args.get_bert_output:  # 如果需要bert的CLS输出
                    ...
                    output_list.extend(pooled_output)  # 加入list
                    #print("bert中得到的特征表示：")
                    #print(len(pooled_output[1]))
                    #print(pooled_output)
        ...
        # todo 将list写入到文件中
        pickle.dump(output_list,open("mnli_20000_feature_main_seed555.pkl","wb"))
        # todo
